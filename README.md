<!-- Presentation -->
<div id="user-content-toc">
  <ul align="center">
    <summary><h1 style="display: inline-block">Hi 👋, I'm Jhonnathan</h1></summary>
  </ul>
</div>

<div id="user-content-toc">
  <ul align="center">
    <summary><h2 style="display: inline-block">Software Engineer | Software Development | TypeScript • Python • React • FastAPI • AWS</h2></summary>
  </ul>
</div>

<!--horizontal divider(gradiant)-->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif">

<!-- About Me -->
I'm a **Software Engineer** passionate about building scalable backend applications, cloud solutions, and distributed systems.

I have professional experience developing REST APIs, full-stack applications, and data processing solutions using technologies such as **Python, FastAPI, Django, AWS, Docker, React, TypeScript, and Vue.js**. Throughout my career, I've worked on software modernization, process automation, and cloud-native applications while collaborating in Agile teams.

Beyond software development, I'm passionate about volunteering and supporting underserved communities, combining technology with social impact.

---

<!-- Intro -->
- 🌱 Currently learning **ROR** and **Angular**.
- 💬 Ask me about **Python, FastAPI, Django, AWS, Docker, REST APIs, and Backend Development**
- 📫 Reach me at **jhonnathanocampo118@gmail.com**
- ⚡ Fun fact: I enjoy hiking, Ultimate Frisbee, Pokémon, and exploring new technologies.

---

<!-- Experience -->
<div id="user-content-toc">
  <ul>
    <summary><h2 style="display: inline-block">💼 Experience</h2></summary>
  </ul>
</div>

- Software Engineering Intern | ISA Energía Colombia (Jul 2025 – Jan 2026)
  - Developed an application that automated SAP data ingestion.
  - Designed ETL pipelines for cleaning, transforming, and visualizing operational data.
  - Modernized and maintained legacy applications to improve stability and maintainability.

- **Junior Software Developer** | Universidad EAFIT (Jul 2024 – Jun 2025)
  - Contributed to full-stack development for urban planning platforms.
  - Developed and maintained RESTful APIs using Django.
  - Utilized Docker for containerization, enhancing scalability and portability.
  
- **Computational Thinking Teacher Assistant** | Universidad EAFIT (Jul 2022 – Jun 2024)
  - Mentored students in Python programming and algorithmic problem-solving.
  - Supported course delivery through collaboration with professors.
  - Contributed to creating and improving teaching materials.

---

<!-- Stats & Languages -->
<div align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=jhonnathan93&show_icons=true&hide_border=true&bg_color=273849&title_color=ffffff&icon_color=ffffff&text_color=ffffff" width="47%" />
  
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=jhonnathan93&layout=compact&hide_border=true&bg_color=273849&title_color=ffffff&text_color=ffffff" width="38%" />
</div>

<!-- Streak Stats -->
<p align="center">
  <img src="https://streak-stats.demolab.com/?user=jhonnathan93&hide_border=true&background=273849&ring=ffffff&fire=ffffff&currStreakLabel=ffffff&sideLabels=ffffff&currStreakNum=ffffff&sideNums=ffffff&dates=ffffff"/>
</p>


<!-- Technologies -->
<div id="user-content-toc">
  <ul>
    <summary><h2 style="display: inline-block">Tech Stack</h2></summary>
  </ul>
</div>

<!-- Programming Languages -->
<h3>Programming Languages</h3>
<p>
  <img src="https://skillicons.dev/icons?i=java,js,php,py,ts&perline=14" />
</p>

---

<!-- Frontend Development -->
<h3>Frontend Development</h3>
<p>
  <img src="https://skillicons.dev/icons?i=cypress,nextjs,react,vue&perline=14" />
</p>

---

<!-- Backend Development -->
<h3>Backend Development</h3>
<p>
  <img src="https://skillicons.dev/icons?i=django,express,fastapi,flask,nodejs,spring,laravel&perline=14" />
</p>

---

<!-- Frontend -->
<h3>Frontend</h3>
<p>
<img src="https://skillicons.dev/icons?i=vue,react,tailwind,html,css&perline=10" />
</p>

---

<!-- Databases -->
<h3>Databases</h3>
<p>
 <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,redis&perline=10" />
</p>

---

<!-- DevOps / Cloud -->
<h3>DevOps & Cloud</h3>
<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,terraform,githubactions,rabbitmq,nginx&perline=10" />
</p>

---

<!-- Tools & Frameworks -->
<h3>Tools & Frameworks</h3>
<p>
  <img src="https://skillicons.dev/icons?i=git,github,postman,vscode&perline=14" />
</p>


---

<!-- Connect -->
<div id="user-content-toc">
  <ul align="center">
    <summary><h2 style="display: inline-block">🤝 Connect With Me</h2></summary>
  </ul>
</div>

<p align="center">
  <a href="https://www.linkedin.com/in/jhonnathan-ocampo" target="blank">
    <img align="center" src="https://user-images.githubusercontent.com/88904952/234979284-68c11d7f-1acc-4f0c-ac78-044e1037d7b0.png" alt="linkedin" height="50" width="50" />
  </a>
  <a href="https://www.instagram.com/jhonnathan.ocampo/" target="blank">
    <img align="center" src="https://user-images.githubusercontent.com/88904952/234981169-2dd1e58f-4b7e-468c-8213-034ba62156c3.png" alt="instagram" height="50" width="50" />
  </a>
</p>
